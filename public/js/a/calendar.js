...
var _calendarFormat = { 'dateOnly':true };

var _calendarMonthArray = [ 'JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC' ];

function calendarStringToDate( s ) {

	let m = s.match(/ *([0-9]+)\-([0-9]+)\-([0-9]+) + ([0-9]+)\:([0-9]+)\:([0-9]+)/);
	if( m !== null ) {
		if( m.length == 7 ) {
...
function calendarDateTimeToString( date, addSeconds=true ) {
	let month = date.getMonth()+1;
	let day = date.getDate();
	let hour = date.getHours();
	let minute = date.getMinutes();
	return calendarToString( date.getFullYear(), month, day, hour, minute, addSeconds );
}


function calendarDateToString( date ) {
	let month = date.getMonth()+1;
	let day = date.getDate();
	return calendarToString( date.getFullYear(), month, day );
}


function calendarToString( year=null, month=null, day=null, hour=null, minute=null, addSeconds=true ) {
	let monthPadding = (month < 10) ? '0' : '';
	let dayPadding = (day < 10) ? '0' : '';
	let date = year + '-' + monthPadding + month + '-' + dayPadding + day;
	if( hour !== null && minute !== null ) {
		let hourPadding = (hour < 10) ? '0' : '';
		let minutePadding = (minute < 10) ? '0' : '';
		date += ' ' + hourPadding + hour + ":" + minutePadding + minute;
		if( addSeconds )
			date += ":00"; 
	}
	return date;
...
function calendarAddHours( date, hours ) {
