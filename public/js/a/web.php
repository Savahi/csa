...
Route::get('/a_users/{param?}', function ($param=NULL) {
	...
	$where = "";
	if( $param == NULL ) {
		$select = "id, name, balance, deposit, email, delivery_point_id, contacts, icon ";
		$where = "admin_privilegies=0 AND delivery_point_id > 0 AND farm_admin=-1 AND delivery_unit_admin=-1 AND sorting_station_admin=-1";
	} else if( $param == 'all' ) {  
		$select = "id, name, balance, deposit, email, contacts, icon, delivery_point_id, delivery_point_admin, delivery_unit_admin, farm_admin, admin_privilegies";
		$where = "";
	} else if( $param == 'delivery_point_admin' ) {  
		$select = "id, name, balance, email, delivery_point_id, contacts, icon, delivery_point_admin";
		...
		$select = "id, name, email, contacts, icon, farm_admin";
		$where = "farm_admin <> -1";
	} else if( $param == 'delivery_unit_admin' ) {  
		$select = "id, name, email, contacts, icon, delivery_unit_admin";
...
Route::get('/a_users/refills/{user_id}', function ($user_id) {
	confirmAdminPrivilegies();
	$query = "SELECT id, title, amount, made_at FROM `refills` WHERE user_id=" . $user_id;
