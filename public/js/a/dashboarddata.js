...
var _aDashboardFieldTitles = { id:'#', 
	...
	contacts: _myStrContacts, deposit: _myStrDeposit,
	...
	created_at:'<span class="glyphicon glyphicon-calendar"></span>',
	price_per_user:'<span class="glyphicon glyphicon-usd"></span><span style="margin:0px 4px 0px 4px; font-size:120%; font-weight:bold;">/</span><span class="glyphicon glyphicon-user"></span>',
	...
	is_delivered:'<span class="glyphicon glyphicon-thumbs-up"></span>', 
    pickup_info:'<span class="glyphicon glyphicon-shopping-cart"></span><span class="glyphicon glyphicon-info-sign"></span>',
	delivery_info:'<span class="glyphicon glyphicon-map-marker"><span class="glyphicon glyphicon-info-sign"></span>',
	minbalance: `${_myStrBalance} &ge;`,
	mindeposit: `${_myStrDeposit} &ge;`
 }; 
...
function aDisplayDashboardDataArray( linkElem, dataArray, params, menuElem=null ) {
	...
	if( !('display' in params) )
		params.display={};
	if( !('tableActions' in params) )
		params.tableActions=[];
	if( !('rowActions' in params) )
		params.rowActions=[];
	...
	if( dataArray.length == 0 ) { 	// If no data loaded...
		let cell = document.createElement('div');		
		cell.style.left = '0px'; 
		cell.style.top = '0px';
		cell.style.width = (bbox.width-10) + 'px'; 
		//cell.style.height = defaultColumnTitleHeight.toString() + 'px';  
		cell.innerHTML = `<div style='padding:24px; text-align:center; font-size:120%;'>${_myStrNoData}</div>`;
	...
	// Column titles...
	for( let key in params.display ) {
		let cell = document.createElement('div');		
		let title = null;
		let tooltip = null;
		if( 'display' in params ) {
			if( 'title' in params.display[key] ) {
				title = params.display[key].title;		
			}
			if( 'tooltip' in params.display[key] ) {
				tooltip = params.display[key].tooltip;
			}		
		}
		if( title === null && key in _aDashboardFieldTitles ) {
			title = _aDashboardFieldTitles[key];
		}
		if( title === null ) {
			title = key;
		}
	...
	for( let i = 0 ; i < dataArray.length ; i++ ) {
		...
		/*
		aTableRow.onmouseover = function(e) {
			for( let i = 0 ; i < this.childNodes.length ; i++ )
				this.childNodes[i].style.background= '#d0d0d0';
		};
		aTableRow.onmouseout = function(e) {
			for( let i = 0 ; i < this.childNodes.length ; i++ )
				this.childNodes[i].style.background=null;
		};
		*/
		// Row title
		let cell = document.createElement('div');		
		...
		for( let key in params.display ) {
			...
			if( key === 'icon' ) {
				...
				if( 'load_from_table' in params.display[key] ) {
					cell.innerHTML = `<img ${imageAttrs} src='/scripts/icon.php?t=${params.display.icon.load_from_table}&i=${dataArray[i].id}'>`;
				} else {
					let imageContent = ( dataArray[i][key] ) ? dataArray[i][key] : _myConstEmptyIcon; //_aDashboardDataEditWindowIconEmpty; 
					cell.innerHTML = "<img " + imageAttrs + " src='data:image/jpg;base64, " + imageContent + "'>";
				}
			} else if( key === 'image_url' ) {
				if( dataArray[i][key] ) {
					let imageAttrs = "style='height:60px; width:80px; border-radius:4px;' vspace=2 hspace=2";
 					cell.innerHTML = `<a href='${dataArray[i][key]}' target=_blank><img ${imageAttrs} src='${dataArray[i][key]}'></a>`;
				} else {
					cell.innerHTML = '';
				}
...
function aDisplayDashboardDataArrayEditWindow( titleText='Untitled', keys, dataItem=null, params=null, cb=null ) {
	...
	twoPaneContainer.style = 'box-sizing:border-box; margin:0; padding:2;';
